...
type Screen = JSX.IntrinsicElements["group"] & {
  frame: Frame;
  panel: Panel;
};
...
function Screen({ frame, panel, children, ...props }: Screen) {
...
interface ScreenText extends Screen {
  invert?: boolean;
  text: string;
  x?: number;
  y?: number;
  ...
  useFrame(
    (state) =>
      (textRef.current.position.x =
        x +
        Math.sin(rand + state.clock.elapsedTime / (text.length / 2)) *
          text.length)
  );
  return (
    <Screen {...props}>
      ...
      />
      <color
        args={[invert ? "black" : "#fc3434"]}
        attach="background"
      />
      <ambientLight intensity={0.5} />
      <directionalLight position={[10, 10, 5]} />
      <Text
        color={!invert ? "black" : "#fc3434"}
...
function Leds({ instances }: { instances: ContextType }) {
